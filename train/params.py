...
def parse_args():
    ...
    parser.add_argument(
    "--distributed",
    type=str2bool,  # 使用自定义函数解析布尔值
    required=True,  # 强制要求必须传入该参数（可选，根据需求决定）
    help="Enable distributed training (True/False)"
    )


    parser.add_argument('--biolord_checkpoint', type=str, default=None, 
                    help='Path to biolord checkpoint')
    # Exp ID
