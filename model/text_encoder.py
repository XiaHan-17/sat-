...
class Text_Encoder(nn.Module):
    def __init__(self, 
        ...
        if text_encoder == 'ours':
            model = class_name(biolord_checkpoint=biolord_checkpoint)
        else:
            model = class_name()
    ...
    def forward(self, label_name, modality_name):
        ...
        modality_code = torch.tensor([modality_code_dict[mod] for mod in modality], device=self.device)   # bn
